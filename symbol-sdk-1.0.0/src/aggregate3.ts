...
const main = async () => {
    ...
    const account3: Account = Account.createFromPrivateKey(`4${privateKey.substr(1)}`, nt);
    ...
    const tx3: TransferTransaction = TransferTransaction.create(
        Deadline.create(ea),
        Address.createFromRawAddress(toAddress),
        [new Mosaic(nc.currency.unresolvedMosaicId, UInt64.fromNumericString(amount))],
        PlainMessage.create(message),
    ...
    const tx: AggregateTransaction = AggregateTransaction.createComplete(
        ...
        [
            ...
            tx3.toAggregate(account3.publicAccount)
        ],
        nt,
        [],
    ...
    const signedTx1: SignedTransaction = account1.signTransactionWithCosignatories(tx, [account2, account3], gh)

    const signedTx2: SignedTransaction = account1.signTransactionWithCosignatories(tx, [account2], gh)
