...
async function exec(version) {
    ...
    const secretLockTxPayload = 
        ...
        "00" +                                              // hashtype
        secret.substr(0,(32*2)) +        // secret
        tx1Signed.payload.substr(((-25)*2));                // recipient
    ...
    request({
        ...
        json: {"payload": signedSecretLockTxPayload}
    }, (error, response, body) => {
        // console.log(body);
    });
    ...
    request({
        url: `${ENDPOINT}/transaction/${signedSecretLockTxHash}/status`,
        method: 'GET',
        headers: {
            'Content-Type':'application/json'
        }
...
(async () => {
    for (let i = 0; i < 256; i++) {
        const n = Number(i).toString(16).toUpperCase();
        const n2 = ("0" + n).substr(-2);
        await exec(n2 + "90")
    }
})()