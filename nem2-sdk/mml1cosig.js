...
const leaderPrivateKey = 'DD28C79DB50143CB0A4FB39E00AF1374E702D944A53E4384B64403052DA3CCB2';
...
const leaderAccount = Account.createFromPrivateKey(leaderPrivateKey, NetworkType.MIJIN_TEST);
...
accountHttp.aggregateBondedTransactions(multisigTeamAccount)
    .flatMap((_) => _)
    .filter((_) => !_.signedByAccount(leaderAccount.publicAccount))
    .map(transaction => cosignAggregateBondedTransaction(transaction, leaderAccount))
    .flatMap(cosignatureSignedTransaction => transactionHttp.announceAggregateBondedCosignature(cosignatureSignedTransaction))
    .subscribe(announcedTransaction => console.log(announcedTransaction),
        err => console.error(err));