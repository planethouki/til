...
// edge
setValue(newfile, 0, 0, 0);
setValue(newfile, max, 0, 0);
setValue(newfile, max, max, 0);
setValue(newfile, 0, max, 0);
// center
setValue(newfile, max / 2, max / 2, 1);
...
for (let x = 0; x < nx; x++) {
  for (let y = 0; y < ny; y++) {
    const v = getValue(newfile, x, y);
    const v2 = Math.floor(v * 5) / 5;
    setValue(newfile, x, y, v2);
  }
}

newfile
  .pack()
  .pipe(fs.createWriteStream(__dirname + "/sample4.png"))
  .on("finish", function () {
    console.log("Written!");
  });
